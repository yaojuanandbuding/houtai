...
1:scoped属性作用：------加上scoped的作用只是对于当前组件有用（样式）
对于某一个组件，如果style添加scoped属性，给当前子组件结构中都添加上应该data-v-xxx自定义属性
...
注意：
如果父组件样式（scoped），还想影响子组件的样式？这种情况我们可以选择深度选择器
...
项目描述:仿京东商城。包括首页、搜索列表、商品详情、购物车、订单、支付、用户登录以及注册等多个子模块。

1、使用Vue全家桶+ES6+Webpack+Axios等技术。

2、使用vue-lazyload实现图片懒加载；通过import()实现路由懒加载。

3、抽取了三级联动和分页器作为公用组件。

4、使用Vue内置组件transition实现三级联动的动画效果。

电商后台管理系统

项目描述：对商城的数据进行管理，如商品管理，对数据进行修改和下架操作。

1.使用Vue+ElementUI开发页面和功能业务组件的封装。

2.使用ECharts，实现数据可视化。

3.实现路由懒加载。

1、熟练HTML,CSS，熟练页面的结构布局，原生JavaScript，熟悉 W3C标准。

2、熟练使用Vue全家桶，Vue，Vuex，Vue-Router 

3、熟练jQuery 

4、熟练使用Swiper等插件使用，以及Element-UI的 使用

5、熟悉canvas画布，svg的基本使用以及ECharts的数据可视化，能够用canvas,svg实现简单的动画效果。

自学能力强，性格开朗拥有较强的适应能力；沟通能力强，积极主动不话唠；技术热情高，具有较好的抗压能力；有问题不逃避，愿意虚心向其他人学习请教。
大学期间动手能力强，自主学习了学校课程中并安排的技术与相关知识，并采取了实践。
