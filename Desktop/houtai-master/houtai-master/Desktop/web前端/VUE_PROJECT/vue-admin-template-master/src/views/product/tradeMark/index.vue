  ...
  <div>
    ...
    <!-- 
        ...
        当前第几页、数据总条数、每一页展示的条数 、连续页码数  
          ...
          page-sizes：代表设置每一页展示多少条数据
          layout：实现分页器布局
          page-count:按钮数量  如果  9  连续页面是7
     -->
    <el-pagination
    ...
    <el-dialog :title="tmForm.id?'修改品牌':'添加品牌'" :visible.sync="dialogFormVisible">
      <!-- form表单  :model把表单的数据收集到哪个对象身上，将来表单验证，也需要这个属性 -->
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules:{
        //品牌名称的验证规则
        //require:必须要校验的字段（前面五角星有关系）message：提示的信息  trigger：用户行为设置（事件的设置：blur）
          tmName: [
  ...
  methods: {
    ...
    async getPageList(pager = 1) {
      ...
      //获取品牌列表的接口
      //当你向服务器发请求的时候，这个函数需要带参数，因此老师在data初始化两个字段，代表给服务器传递的参数
